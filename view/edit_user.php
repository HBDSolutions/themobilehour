...
/*session_start();
if (!isset($_SESSION['user']) || $_SESSION['permissionsID'] < 3) {
    // Only allow Administration Manager users
    ...
    <body>
        ...
        <main>
            ...
            <!-- Edit an existing user form. Ensure the enctype is included to permit file upload -->
                <h1 class="first_row">Edit User</h1>
                <?php
                    if (isset($_GET['id'])) {
                        $id = $_GET['id'];
                    }
                ...
                ?>
                <form class="item-form" enctype="multipart/form-data" action="../controller/edituser.php?id=<?php echo $id?>" method="POST">
                    <?php
                            ...
                            <select id="permissionsID" name="permissionsID" class="form-control" required>
                                <?php
                                    ...
                                    foreach ($rolesStmt as $role) {
                                        $selected = ($role['permissionsID'] == $row['permissionsID']) ? 'selected' : '';
